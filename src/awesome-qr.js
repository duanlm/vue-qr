...
(function() {
  ...
  var Drawing = !_isSupportCanvas() ? (function() {
      ...
      for (var row = 0; row < nCount; row++) {
        ...
        for (var col = 0; col < nCount; col++) {
          aHTML.push(
            ...
            nWidth + 'px;height:' + nHeight + 'px;background-color:' +
    ...
    if (this._android && this._android <= 2.1) {
      ...
      CanvasRenderingContext2D.prototype.drawImage = function(image, sx,
    ...
    Drawing.prototype.draw = function(oQRCode) {
      ...
      var rawViewportSizeWidthAll = widthAll - 2 * rawMargin;
      var rawViewportSizeHeightAll = heightAll - 2 * rawMargin;
      var rawWidthAll = rawViewportSizeWidthAll / nCount;
      var rawHeightAll = rawViewportSizeHeightAll / nCount;
      var nWidthAll = Math.ceil(rawWidthAll);
      var nHeightAll = Math.ceil(rawHeightAll);
      var viewportSizeWidthAll = nWidthAll * nCount;
      ...
      _bkgCanvas.width = sizeWidthAll;
      _bkgCanvas.height = sizeHeightAll;
      // 背景笔
      ...
      if (_htOption.backgroundImage !== undefined) {
        if (_htOption.autoColor) {
          ...
          _htOption.colorDark = "rgb(" + avgRGB.r + ", " + avgRGB.g +
            ", " + avgRGB.b + ")";
        }
        ...
        if (_htOption.maskedDots) {
           ...
           */
          _mContext.drawImage(_htOption.backgroundImage,
            0, 0, _htOption.backgroundImage.width, _htOption.backgroundImage
            .height,
            0, 0, size, size);
          ...
          _bContext.drawImage(_htOption.backgroundImage,
            0, 0, _htOption.backgroundImage.width, _htOption.backgroundImage
            .height,
            0, 0, sizeWidthAll, sizeHeightAll);
          _bContext.rect(0, 0, sizeWidthAll, sizeHeightAll);
      ...
      for (var row = 0; row < nCount; row++) {
        for (var col = 0; col < nCount; col++) {
          ...
          var isBlkPosCtr = ((col < 8 && (row < 8 || row >= nCount - 8)) ||
            (col >= nCount - 8 && row < 8));
          var isBlkPos = ((col < 7 && (row < 7 || row >= nCount - 7)) ||
            (col >= nCount - 7 && row < 7));
          var bProtected = (row === 6 || col === 6 || isBlkPosCtr);

          for (var i = 0; i < agnPatternCenter.length - 1; i++) {
            bProtected = bProtected || (row >= agnPatternCenter[i] - 2 &&
              row <= agnPatternCenter[i] + 2 && col >=
              agnPatternCenter[i] - 2 && col <= agnPatternCenter[i] +
              2);
          }

          var nLeft = col * nWidth + (bProtected ? 0 : (xyOffset *
            nWidth));
          var nTop = row * nHeight + (bProtected ? 0 : (xyOffset *
            nHeight));
          ...
          _oContext.fillStyle = bIsDark ? _htOption.colorDark :
            "rgba(255, 255, 255, 1)"; //_htOption.colorLight;
          if (agnPatternCenter.length === 0) {
            // if align pattern list is empty, then it means that we don't need to leave room for the align patterns
            if (!bProtected)
              fillRect_(_oContext, nLeft, nTop, (bProtected ? (
                  isBlkPosCtr ? 1 : 1) : dotScale) * nWidth, (
                  bProtected ? (isBlkPosCtr ? 1 : 1) : dotScale) *
                nHeight, _maskCanvas, bIsDark);
          } else {
            var inAgnRange = ((col < nCount - 4 && col >= nCount - 4 -
              5 && row < nCount - 4 && row >= nCount - 4 - 5));
      ...
      var protectorStyle = "rgba(255, 255, 255, 1)";
      ...
      _oContext.fillRect(0, (nCount - 8) * nHeight, 8 * nWidth, 8 *
        nHeight);
      _oContext.fillRect((nCount - 8) * nWidth, 0, 8 * nWidth, 8 *
        nHeight);
      _oContext.fillRect(8 * nWidth, 6 * nHeight, (nCount - 8 - 8) *
        nWidth, nHeight);
      _oContext.fillRect(6 * nWidth, 8 * nHeight, nWidth, (nCount - 8 -
        8) * nHeight);
      ...
      for (var i = 0; i < agnPatternCenter.length; i++) {
        for (var j = 0; j < agnPatternCenter.length; j++) {
          ...
          if (agnX === 6 && (agnY === 6 || agnY === edgeCenter)) {
            ...
            continue;
          } else if (agnX !== 6 && agnX !== edgeCenter && agnY !== 6 &&
            agnY !== edgeCenter) {
      ...
      _oContext.fillRect((nCount - 7) * nWidth, 6 * nHeight, 7 * nWidth,
        nHeight);
      _oContext.fillRect(0, (nCount - 7) * nHeight, 7 * nWidth, nHeight);
      _oContext.fillRect(0, (nCount - 7 + 6) * nHeight, 7 * nWidth,
        nHeight);
      ...
      _oContext.fillRect((nCount - 7 + 6) * nWidth, 0, nWidth, 7 *
        nHeight);
      _oContext.fillRect(0, (nCount - 7) * nHeight, nWidth, 7 * nHeight);
      _oContext.fillRect(6 * nWidth, (nCount - 7) * nHeight, nWidth, 7 *
        nHeight);

      _oContext.fillRect(2 * nWidth, 2 * nHeight, 3 * nWidth, 3 *
        nHeight);
      _oContext.fillRect((nCount - 7 + 2) * nWidth, 2 * nHeight, 3 *
        nWidth, 3 * nHeight);
      _oContext.fillRect(2 * nWidth, (nCount - 7 + 2) * nHeight, 3 *
        nWidth, 3 * nHeight);

      for (var i = 0; i < nCount - 8; i += 2) {
        _oContext.fillRect((8 + i) * nWidth, 6 * nHeight, nWidth,
          nHeight);
        _oContext.fillRect(6 * nWidth, (8 + i) * nHeight, nWidth,
          nHeight);
      }
      for (var i = 0; i < agnPatternCenter.length; i++) {
        for (var j = 0; j < agnPatternCenter.length; j++) {
      ...
      if (_htOption.logoImage !== undefined) {
        ...
        _oContext.fillStyle = '#FFFFFF';
        _oContext.save();
        prepareRoundedCornerClip(_oContext, x - logoMargin, y -
          logoMargin, logoSize + 2 * logoMargin, logoSize + 2 *
          logoMargin, logoCornerRadius);
        _oContext.clip();
        _oContext.fill();
        _oContext.restore();

        _oContext.save();
        prepareRoundedCornerClip(_oContext, x, y, logoSize, logoSize,
          logoCornerRadius);
        _oContext.clip();
        _oContext.drawImage(_htOption.logoImage, x, y, logoSize,
          logoSize);
        _oContext.restore();
      }
      _bContext.drawImage(_tCanvas, xx, yy, sizeWidthAll, sizeHeightAll);
      _oContext.drawImage(_bkgCanvas, -margin, -margin, sizeWidthAll,
        sizeHeightAll);
      if (_htOption.binarize) {
        var pixels = _oContext.getImageData(0, 0, size, size);
        var threshold = 128;
        if (parseInt(_htOption.binarizeThreshold) > 0 && parseInt(
            _htOption.binarizeThreshold) < 255) {
      ...
      _fCanvas.width = sizeWidthAll;
      _fCanvas.height = sizeHeightAll;
      _fContext.drawImage(_tCanvas, 0, 0, sizeWidthAll, sizeHeightAll);
    ...
    Drawing.prototype.clear = function() {
      this._oContext.clearRect(0, 0, this._elCanvas.width, this._elCanvas
        .height);
      this._bIsPainted = false;
    };
  ...
  function fillRect_(canvas, x, y, w, h, maskSrc, bDark) {
    ...
    if (maskSrc === undefined) {
      ...
      canvas.fillStyle = bDark ? "rgba(0, 0, 0, .5)" :
        "rgba(255, 255, 255, .7)";
      canvas.fillRect(x, y, w, h);
  ...
  function drawAgnProtector(context, centerX, centerY, nWidth, nHeight) {
    context.clearRect((centerX - 2) * nWidth, (centerY - 2) * nHeight, 5 *
      nWidth, 5 * nHeight);
    context.fillRect((centerX - 2) * nWidth, (centerY - 2) * nHeight, 5 *
      nWidth, 5 * nHeight);
  }

  function drawAgn(context, centerX, centerY, nWidth, nHeight) {
    context.fillRect((centerX - 2) * nWidth, (centerY - 2) * nHeight, nWidth,
      4 * nHeight);
    context.fillRect((centerX + 2) * nWidth, (centerY - 2 + 1) * nHeight,
      nWidth, 4 * nHeight);
    context.fillRect((centerX - 2 + 1) * nWidth, (centerY - 2) * nHeight, 4 *
      nWidth, nHeight);
    context.fillRect((centerX - 2) * nWidth, (centerY + 2) * nHeight, 4 *
      nWidth, nHeight);
  ...
  AwesomeQRCode.prototype.makeImage = function() {
    if (typeof this._oDrawing.makeImage == "function" && (!this._android ||
        this._android >= 3)) {
  ...
  function getAverageRGB(imgEl) {
    ...
    height = canvas.height = imgEl.naturalHeight || imgEl.offsetHeight ||
      imgEl.height;
    ...
    while ((i += blockSize * 4) < length) {
      if (data.data[i] > 200 || data.data[i + 1] > 200 || data.data[i + 2] >
        200) continue;
      ++count;
      rgb.r += data.data[i];
      rgb.g += data.data[i + 1];
